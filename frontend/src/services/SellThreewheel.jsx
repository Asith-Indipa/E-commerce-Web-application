...
export default function SellThreewheel() {
  const [location, setLocation] = useState("Kamburupitiya");
  const [category, setCategory] = useState("Three Wheelers");
  ...
  useEffect(() => {
    ...
    const fetchBrands = async () => {
      try {
        ...
        const data = await res.json();
        const threeBrands = Array.from(
          new Set(
            data
              .filter((item) => item.category === "Three Wheelers")
              .map((item) => item.brand)
          )
        );
        setBrands(threeBrands);
      } catch {
        setBrands([]);
      }
    };
  ...
  useEffect(() => {
    const fetchModels = async () => {
      try {
        const res = await fetch(`${BASE_URL}/api/vehiclemodelbrand/all`);
        const data = await res.json();
        const threeModels = Array.from(
          new Set(
            data
              .filter((item) => item.category === "Three Wheelers" && item.brand === brand)
              .map((item) => item.model)
          )
        );
